...
Код для разбиение на чанки и создания векторной базы данных python ingest.py 

## Запуск MCP-сервера

В проекте используется локальный MCP-сервер:

local_mcp_server.py

Он предоставляет инструмент:
...
Агент использует GigaChat и инструмент MCP qdrant_find.
...
Код для запуска: python evaluate.py
